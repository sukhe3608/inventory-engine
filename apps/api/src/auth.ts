...
const keyLimits = new Map<string, { count: number; resetAt: number }>();

function checkKeyLimit(keyId: string): boolean {
  const now = Date.now();
...
export async function requireScopes(scopes: string[]) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { rows } = await db.query<KeyRow>(
      `select k.id, k.tenant_id, k.scopes, k.name, k.revoked_at, t.status as tenant_status, k.last_used_at
