...
export const invitationsRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.post('/v1/invitations/:id/send-email', async (request, reply) => {
    ...
    const {
      ...
      `select i.id, i.tenant_id, i.email, i.role, i.token, i.status, t.name as tenant_name
  ...
  <p style="color:#94a3b8;font-size:12px;margin-top:24px">Inventory Engine · secure multi-tenant inventory</p>
      ...
      return reply.code(500).send({ error: 'EMAIL_SEND_FAILED', detail: (err as Error).message });
    }
