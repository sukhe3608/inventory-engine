...
export const ordersRoutes = async (app) => {
    app.get('/v1/orders', { preHandler: await requireScopes(['orders:read']) }, async (request, reply) => {
        const apiKey = request.apiKey;
        const { rows } = await app.db.query(`select o.*,
           coalesce((
         ...
         limit 200`, [apiKey.tenantId]);
        return { orders: rows, count: rows.length };
    });
    app.post('/v1/orders', { preHandler: await requireScopes(['orders:write']) }, async (request, reply) => {
        const apiKey = request.apiKey;
        ...
        for (const item of body.items) {
            if (!isUuid(item.variant_id))
                return reply.code(400).send(validationError('INVALID_VARIANT_ID'));
            if (item.location_id && !isUuid(item.location_id))
                return reply.code(400).send(validationError('INVALID_LOCATION_ID'));
        }
        try {
            const { rows } = await app.db.query(`select * from public.create_order(
    ...
    app.post('/v1/orders/:id/cancel', { preHandler: await requireScopes(['orders:write']) }, async (request, reply) => {
        const apiKey = request.apiKey;
        ...
        try {
            const { rows } = await app.db.query(`select * from public.update_order_status($1::uuid, 'cancelled', null, $2::text, $3::uuid)`, [id, 'api:' + apiKey.name, apiKey.tenantId]);
