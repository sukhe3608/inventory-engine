...
function buildFilters(tenantId, query) {
    const clauses = ['sl.tenant_id = $1'];
    const params = [tenantId];
    if (query.variant_id) {
        if (!isUuid(query.variant_id))
            return { error: 'INVALID_VARIANT_ID' };
        params.push(query.variant_id);
        clauses.push(`sl.variant_id = $${params.length}`);
    }
    if (query.location_id) {
        if (!isUuid(query.location_id))
            return { error: 'INVALID_LOCATION_ID' };
...
export const stockRoutes = async (app) => {
    app.get('/v1/stock', { preHandler: await requireScopes(['stock:read']) }, async (request, reply) => {
        ...
        const { rows } = await app.db.query(`select
         ...
         order by v.sku, l.name`, filters.params);
        return { stock: rows, count: rows.length };
    });
    app.get('/v1/stock/history', { preHandler: await requireScopes(['stock:read']) }, async (request, reply) => {
        const apiKey = request.apiKey;
    ...
    const handleStockWrite = (transactionType) => async (request, reply) => {
        const apiKey = request.apiKey;
        ...
        try {
            const { rows } = await app.db.query(`select * from public.apply_stock_change(
             $1::uuid, $2::uuid, $3::numeric, $4::text, null, null, $5::text, $6::text,
             null, $7::text, $8::uuid)`, [body.location_id, body.variant_id, body.change_qty, transactionType,
    ...
    app.post('/v1/stock/adjustments', { preHandler: await requireScopes(['stock:write']) }, handleStockWrite('adjustment'));
    app.post('/v1/stock/opening-balances', { preHandler: await requireScopes(['stock:write']) }, handleStockWrite('opening_balance'));
};
